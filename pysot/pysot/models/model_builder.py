...
class ExportModelBuilder(ModelBuilder):
    ...
    def __init__(self, state_dict=None, anchor=None):
        super(ExportModelBuilder, self).__init__()
        if state_dict is not None:
            self.load_state_dict(state_dict)
        self.eval()

        self.score: DepthwiseXCorr = self.rpn_head.cls
        self.pred_bbox: DepthwiseXCorr = self.rpn_head.loc
    ...
    @staticmethod
    def _convert_bbox_torch(delta, anchor):
        delta = delta.permute(1, 2, 3, 0).contiguous().view(4, -1)
        x, y, w, h = delta.split((1, 1, 1, 1), 0)  # y.tensor_split((2, 4, 5), 4)# torch 1.8.0

        x = x * anchor[:, 2] + anchor[:, 0]
        y = y * anchor[:, 3] + anchor[:, 1]
        w = torch.exp(w) * anchor[:, 2]
        h = torch.exp(h) * anchor[:, 3]

        return torch.cat([x, y, w, h], 0)

    @staticmethod
    def _convert_score_torch(score):
    ...
    def template_forward(self, z):
        out = self.backbone(z)
        self.cls_kernel = self.score.conv_kernel(out)
        self.loc_kernel = self.pred_bbox.conv_kernel(out)
        return self.cls_kernel, self.loc_kernel

    def track1(self, x):
        out = self.backbone(x)
        cls_search = self.score.conv_search(out)
        loc_search = self.pred_bbox.conv_search(out)
    ...
    def track2(self, cls_feature, loc_feature):
        score = self.score.head(cls_feature)
        pred_bbox = self.pred_bbox.head(loc_feature)
        return score, pred_bbox

    def track_full(self, x, cls_kernel, loc_kernel):
        cls_search, loc_search = self.track1(x)
        cls_feature = xcorr_depthwise(cls_search, cls_kernel)
        loc_feature = xcorr_depthwise(loc_search, loc_kernel)
        score, pred_bbox = self.track2(cls_feature, loc_feature)
        score = self._convert_score_torch(score)
        pred_bbox = self._convert_bbox_torch(pred_bbox, self.anchor)
        return score, pred_bbox
    ...
    def export_onnx(self, template, x, dirname, mnn=False):
        import json
        ...
        track_full_input_names = ["x", "cls_k", "loc_k"]
        track1_output_names = ["cls_s", "loc_s"]
        track2_input_names = ["cls_f", "loc_f"]
        track2_output_names = ["score", "pred_bbox"]
        names = {
            ...
            "track_full": {"input_names": track_full_input_names, "output_names": track2_output_names}
        }
        ...
        if not mnn:
            ...
            logger.info("2. export track full for MNN.")

            cls_k, loc_k = self(template)
            filename2 = osp.join(dirname, "track_full.onnx")
            self.export_one_part(self.track_full, [x, cls_k, loc_k], filename2, names["track_full"])
            return [filename1, filename2], [names["template"], names["track_full"]]
    ...
    def start_export(self, template, x, dirname, trt=True, mnn=False):

        if trt:
            self.export_trt(*self.export_onnx(template, x, dirname))
        else:
            files, _ = self.export_onnx(template, x, dirname, mnn)
            [self.onnx_simplify(file) for file in files]
...
class ExportModelBuilder2(ModelBuilder):
    ...
    def track_full(self, x, cls1_k, cls2_k, cls3_k, loc1_k, loc2_k, loc3_k):
        (cls1_s, cls2_s, cls3_s), (loc1_s, loc2_s, loc3_s) = self.track1(x)
        ...
        score, pred_bbox = self.track2(*clss_f, *locs_f)
        score = self._convert_score_torch(score)
        pred_bbox = self._convert_bbox_torch(pred_bbox, self.anchor)
        return score, pred_bbox
    ...
    def export_onnx(self, template, x, dirname, mnn=False):
        ...
        track_full_input_names = ["x", "cls1_k", "cls2_k", "cls3_k", "loc1_k", "loc2_k", "loc3_k"]
        track_full_output_names = ["score", "pred_bbox"]

        names = {
            "template": {"input_names": template_input_names, "output_names": template_output_names},
            "track1": {"input_names": track1_input_names, "output_names": track1_output_names},
            "track2": {"input_names": track2_input_names, "output_names": track2_output_names},
            "track_full": {"input_names": track_full_input_names, "output_names": track_full_output_names}
        }
        ...
        if not mnn:
            ...
            logger.info("2. export track full for MNN.")
            filename2 = osp.join(dirname, "track_full.onnx")
            self.export_one_part(self.track_full, [x, cls1_k, cls2_k, cls3_k, loc1_k, loc2_k, loc3_k], filename2, names["track_full"])
            return [filename1, filename2], [names["template"], names["track_full"]]
    ...
    def start_export(self, template, x, dirname, trt=True, mnn=False):
        ...
        if trt and not mnn:
            ...
            self.export_onnx(template, x, dirname, mnn)